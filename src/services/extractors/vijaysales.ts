...
export async function extractVijaySales(browser: Browser, url: string): Promise<ExtractorResult> {
  console.log(`[Vijay Sales Extractor] Scraping URL: ${url}`);
  ...
  try {
    try {
      ...
      console.log(`[Vijay Sales Extractor] page.goto warning: ${e.message}. Continuing.`);
    }
    
    try {
      await page.waitForSelector('.pdp-price, .vspill, .vsd-price, span.price, .price-display, #product-price, .product__price--price, [type="application/ld+json"]', { timeout: 2000 });
    } catch (e) {
      // Continue anyway
    }
    ...
    const isOOS = await page.evaluate(() => {
      const buyButton = document.querySelector('#btnaddtocart, .btn-add-to-cart, button[class*="add-to-cart"]');
      const buyButtonText = buyButton && buyButton.textContent ? buyButton.textContent.toLowerCase() : '';
      const pdpText = document.querySelector('.pdp-right, .product-details-container')?.textContent?.toLowerCase() || '';
      return buyButtonText.includes('out of stock') || buyButtonText.includes('sold out') || pdpText.includes('temporarily out of stock');
    });

    if (isOOS) {
      console.log('[Vijay Sales Extractor] Product is out of stock. Returning null price.');
    ...
    const domData = await page.evaluate(() => {
      const priceEl = document.querySelector('#product-price, .product__price--price, .product__price--price.offer, .pdp-price, .vspill, .vsd-price, span.price, .price-display');
      const ratingEl = document.querySelector('.pdp-rating, .rating-count, .vs-rating-num');
    ...
    if (domData.price) {
      ...
      if (normP) {
        console.log(`[Vijay Sales Extractor] CSS Selector matched: Price=₹${normP.price}, Rating=${normR}`);
    ...
    if (schemaData && schemaData.price) {
      ...
      if (normP) {
        console.log(`[Vijay Sales Extractor] Schema matched: Price=₹${normP.price}, Rating=${normR}`);
    ...
    for (const text of bodyHeuristics) {
      ...
      if (normP) {
        console.log(`[Vijay Sales Extractor] Heuristic matched: Price=₹${normP.price}`);
    ...
    console.error(`[Vijay Sales Extractor] Error: ${err.message}`);
